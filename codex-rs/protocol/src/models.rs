...
impl FunctionCallOutputPayload {
    ...
    /// Mutable text accessor kept for the local privacy gateway, which restores
    /// redacted function-call output text in place. Upstream removed the helper
    /// as unused (#34345); nothing upstream calls it, but `core::privacy_gateway`
    /// does, so the composed tree keeps it rather than dropping the local call.
