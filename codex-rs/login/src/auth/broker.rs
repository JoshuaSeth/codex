...
fn parse_heartbeat_interval(value: String) -> Result<u64, AuthBrokerError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn acquire_lease_decodes_auth_json() {
        ...
        let client = AuthBrokerClient {
            config: AuthBrokerConfig {
                ...
                lease_reason: "test".to_string(),
                heartbeat_interval: std::time::Duration::from_secs(30),
            },
            http: reqwest::Client::new(),
        };

        let lease = client.acquire_lease("affinity").await.unwrap();

        assert_eq!(lease.lease_id, "lease-1");
        assert_eq!(lease.account_id, "account-1");
        assert_eq!(lease.auth, auth);
    }
}
