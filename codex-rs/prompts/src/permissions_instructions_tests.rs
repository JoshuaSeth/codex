...
#[test]
fn catalog_approval_admission_is_byte_bounded_for_both_reviewers() {
    for reviewer in [ApprovalsReviewer::User, ApprovalsReviewer::AutoReview] {
        let legacy = approval_text(
            AskForApproval::OnRequest,
            reviewer,
            None,
            &Policy::empty(),
        ...
        for selected in [
            String::new(),
            "a".repeat(MAX_CATALOG_APPROVAL_BYTES - 1),
            "a".repeat(MAX_CATALOG_APPROVAL_BYTES),
            "a".repeat(MAX_CATALOG_APPROVAL_BYTES + 1),
            "é".repeat(MAX_CATALOG_APPROVAL_BYTES / 2),
            "é".repeat(MAX_CATALOG_APPROVAL_BYTES / 2 + 1),
        ] {
            let oversized_other = Some("x".repeat(MAX_CATALOG_APPROVAL_BYTES + 1));
            let messages = match reviewer {
