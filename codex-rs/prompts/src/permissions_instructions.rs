...
// Local renderer admission limit; this does not bound catalog ingestion.
// Reject oversized guidance intact rather than truncating permission rules.
const MAX_CATALOG_APPROVAL_BYTES: usize = 16 * 1024;
...
fn approval_text(
        ...
        && let Some(approval_messages) = approval_messages
    {
        ...
        if let Some(selected) = selected
            && selected.len() <= MAX_CATALOG_APPROVAL_BYTES
        {
