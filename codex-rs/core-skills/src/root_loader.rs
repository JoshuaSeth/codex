...
/// Load every root and merge the snapshots. Upstream's entry point: callers
/// that hold a scan pool pass it here and name precedence stays off.
...
{
    load_and_merge_skill_roots_with_name_precedence(
        roots,
...
/// Load every root and merge the snapshots, optionally letting a tenant root
/// shadow the roots it covers. One implementation serves both entry points.
pub(crate) async fn load_and_merge_skill_roots_with_name_precedence<I>(
    roots: I,
    plugin_skill_snapshots: Option<&PluginSkillSnapshots>,
