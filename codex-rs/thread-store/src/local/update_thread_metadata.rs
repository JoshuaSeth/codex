...
pub(super) async fn update_thread_metadata(
    ...
    if paginated && requires_rollout_compat && patch.git_info.is_none() {
        // PitchAI local: a paginated thread keeps its metadata in SQLite, and a Git-metadata
        // patch is applied there directly by the block below. Any other patch that needs the
        // legacy rollout path has no paginated representation, so it fails here, before either
        // persistence layer is mutated, instead of being half-applied.
...
pub(super) async fn apply_thread_metadata_patch(
    ...
    let requires_rollout_compat = requires_rollout_compatibility_update(&params.patch);
    if requires_rollout_compat {
        update_thread_metadata(store, params).await?;
        return Ok(());
    }
    // PitchAI local: this entry point exists so live append paths can synchronize derived
    // metadata without building a materialized thread response. The explicit name update below
    // still needs the canonical mode, and upstream reads it the same way - metadata only, no
    // history - so the local path keeps its no-response contract without guessing the mode.
    let history_mode = if params.patch.name.is_some() {
        Some(
            read_thread::read_thread(
                store,
                ReadThreadParams {
...
async fn apply_thread_git_info_to_rollout(
    ...
    append_rollout_compatibility_item(
        store,
        rollout_path,
        thread_id,
...
async fn append_rollout_compatibility_item(
    store: &LocalThreadStore,
    rollout_path: &Path,
    thread_id: ThreadId,
    item: RolloutItem,
    ...
    if live_thread_loaded {
        return live_writer::append_items(
            store,
