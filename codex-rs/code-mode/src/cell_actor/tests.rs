...
struct HeldNotificationHost {
    events_tx: mpsc::UnboundedSender<HostEvent>,
    notification_release: Notify,
}

impl HeldNotificationHost {
    fn new() -> (Arc<Self>, mpsc::UnboundedReceiver<HostEvent>) {
...
impl CellHost for HeldNotificationHost {
    async fn invoke_tool(
        &self,
        _invocation: CellToolCall,
        _cancellation_token: CancellationToken,
    ...
    async fn notify(
        ...
        let _ = self.events_tx.send(HostEvent::NotificationStarted);
        cancellation_token.cancelled().await;
        let _ = self.events_tx.send(HostEvent::NotificationCancelled);
    ...
    async fn closed(&self) {
        let _ = self.events_tx.send(HostEvent::Closed);
    }
}
...
async fn next_host_event(events_rx: &mut mpsc::UnboundedReceiver<HostEvent>) -> HostEvent {
...
#[tokio::test]
async fn unexpected_runtime_close_cancels_callbacks_before_responding() {
    let (host, mut events_rx) = HeldNotificationHost::new();
    ...
    harness
        ...
        .send(RuntimeEvent::Notify {
            ...
            text: "pending notification".to_string(),
        })
        .unwrap();
    assert_eq!(
        next_host_event(&mut events_rx).await,
        HostEvent::NotificationStarted,
    );

    let CellActorHarness {
        event_tx,
        handle: _handle,
        mut initial_event_rx,
        task,
        runtime_control_rx: _runtime_control_rx,
        _runtime_event_rx,
    } = harness;
    drop(event_tx);
    ...
    assert_eq!(
        next_host_event(&mut events_rx).await,
        HostEvent::NotificationCancelled,
    );
    assert!(
        tokio::time::timeout(Duration::from_millis(20), &mut initial_event_rx)
            .await
            .is_err(),
        "terminal response must wait for callback cleanup",
    );

    host.release_notification();
    assert_eq!(
    ...
    task.await.unwrap();
    assert_eq!(next_host_event(&mut events_rx).await, HostEvent::Closed);
}
