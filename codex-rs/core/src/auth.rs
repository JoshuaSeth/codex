...
impl AuthManager {
    ...
    pub(crate) fn with_broker_for_testing(
        codex_home: PathBuf,
        auth_credentials_store_mode: AuthCredentialsStoreMode,
        broker_url: String,
        broker_token: String,
    ) -> Arc<Self> {
        let mut manager = Self::new(codex_home, false, auth_credentials_store_mode);
        manager.broker_client = Some(AuthBrokerClient::new_for_testing(broker_url, broker_token));
        Arc::new(manager)
    }
