...
impl Session {
    ...
    pub(crate) async fn recommended_plugin_candidates_for_turn(
        &self,
        turn_context: &TurnContext,
    ...
    async fn build_initial_context_with_world_state_and_mcp(
        ...
        let loaded_plugins = self
            ...
            .await;
        let tool_suggest_is_enabled = crate::tools::spec_plan::tool_suggest_enabled(turn_context);
        let auth = if tool_suggest_is_enabled {
            self.services.auth_manager.auth().await
        } else {
            None
        };
        let recommended_plugin_candidates = self
