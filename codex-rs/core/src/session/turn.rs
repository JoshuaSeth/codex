...
pub(crate) async fn built_tools(
    ...
    let endpoint_recommended_plugin_candidates = sess
        .recommended_plugin_candidates_for_turn(turn_context, &loaded_plugins, auth.as_ref())
        .await;
...
async fn try_run_sampling_request(
    ...
    // Upstream derives this from the turn context now (#32900); the pre-unit
    // inline gate read `model_info.supports_reasoning_summaries`, a field this
    // boundary removes, so keeping it verbatim would not compile. The composed
    // turn context's `effective_reasoning_effort()` carries the same
    // effort-or-default computation without the removed field. Ours read
    // `turn_context.collaboration_mode` as a field; upstream turned it into a
    // method, and the same `ModeKind` is carried by `turn_context.mode`.
