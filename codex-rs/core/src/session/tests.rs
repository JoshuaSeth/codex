...
#[tokio::test]
async fn recommended_plugin_candidates_are_filtered_once_per_turn() {
    let server = MockServer::start().await;
    ...
    let initially_loaded = PluginLoadOutcome::default();

    let first = session
        .recommended_plugin_candidates_for_turn(&turn_context, &initially_loaded, auth.as_ref())
        .await;
    assert_eq!(
        first
            .as_deref()
    ...
    let installed = PluginLoadOutcome::from_plugins(vec![codex_core_plugins::LoadedPlugin {
        config_name: "github@openai-curated-remote".to_string(),
        manifest_name: Some("GitHub".to_string()),
        plugin_namespace: None,
        manifest_description: None,
        root: AbsolutePathBuf::try_from(codex_home.path().join("installed-github"))
    ...
    let second = session
        .recommended_plugin_candidates_for_turn(&turn_context, &installed, auth.as_ref())
        .await;

    assert_eq!(second, first, "the turn must reuse its original snapshot");
}
