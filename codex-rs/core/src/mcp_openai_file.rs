...
async fn build_uploaded_argument_value(
    ...
    // Never retry this read without the turn's permission context. Executors that cannot stream
    // through a platform sandbox must fail closed instead of uploading host-visible bytes.
    let contents = fs
...
mod tests {
    ...
    #[tokio::test]
    async fn build_uploaded_argument_value_does_not_bypass_restricted_turn_profile() {
        use wiremock::MockServer;
        ...
        let auth = CodexAuth::create_dummy_chatgpt_auth_for_testing();
        let dir = tempdir().expect("temp dir");
        tokio::fs::write(dir.path().join("private.txt"), b"private")
            .await
            .expect("write local file");
        set_primary_environment_cwd(&mut turn_context, dir.path());
        ...
        .await
        .expect_err("restricted upload must not fall back to an unsandboxed read");
        ...
        assert_eq!(
            server
                ...
                .len(),
            0,
            "a denied filesystem read must not start an upload",
        );
    }
