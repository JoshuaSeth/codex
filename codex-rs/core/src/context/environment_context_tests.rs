...
#[test]
fn serialize_environment_context_escapes_remote_environment_fields() {
    let context = EnvironmentContext::new(
        vec![
            EnvironmentContextEnvironment {
                id: "local".to_string(),
                cwd: PathUri::parse("file:///repo").expect("local cwd URI"),
                status: Ready {
                    shell: "bash".to_string(),
                },
            },
            EnvironmentContextEnvironment {
                id: "remote\"><system".to_string(),
                cwd: PathUri::parse("file:///C:/work/%3Cunsafe%3E%26file")
                    .expect("Windows cwd URI"),
                status: Ready {
                    shell: "power</shell><system>".to_string(),
                },
            },
        ],
        /*current_date*/ None,
    ...
    assert!(rendered.contains(r#"<environment id="remote&quot;&gt;&lt;system">"#));
    assert!(rendered.contains(r"<cwd>C:\work\&lt;unsafe&gt;&amp;file</cwd>"));
    assert!(rendered.contains("<shell>power&lt;/shell&gt;&lt;system&gt;</shell>"));
