...
impl EnvironmentContextEnvironmentStatus {
    ...
    fn render(&self, indent: &str) -> String {
        match self {
            Self::Starting => render_indented_text_element(indent, "status", "starting"),
            Self::Ready { shell } => render_indented_text_element(indent, "shell", shell),
        }
    }
}
...
fn render_indented_text_element(indent: &str, name: &str, value: &str) -> String {
    let mut rendered = indent.to_string();
    push_text_element(&mut rendered, name, value);
    rendered
}
    ...
    fn body(&self) -> String {
        ...
        match &self.environments {
            EnvironmentContextEnvironments::Single(environment) => {
                ...
                lines.push(render_indented_text_element("  ", "cwd", &cwd));
                lines.push(environment.status.render("  "));
            }
            EnvironmentContextEnvironments::Multiple(environments) => {
                lines.push("  <environments>".to_string());
                for environment in environments {
                    let mut environment_open_tag = "    <environment id=\"".to_string();
                    push_xml_escaped_text(&mut environment_open_tag, &environment.id);
                    environment_open_tag.push_str("\">");
                    lines.push(environment_open_tag);
                    let cwd = environment.cwd.inferred_native_path_string();
                    lines.push(render_indented_text_element("      ", "cwd", &cwd));
                    lines.push(environment.status.render("      "));
