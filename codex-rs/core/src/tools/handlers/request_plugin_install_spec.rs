...
mod tests {
    ...
    #[test]
    fn every_presentation_forbids_parallel_install_requests() {
        for presentation in [
            ToolSuggestPresentation::ListTool,
            ToolSuggestPresentation::RecommendationContext,
        ] {
            let ToolSpec::Function(tool) = create_request_plugin_install_tool(presentation) else {
                panic!("expected function tool spec");
            };
            assert!(
                tool.description
                    .contains("IMPORTANT: DO NOT call this tool in parallel with other tools.")
            );
        }
    }
}
