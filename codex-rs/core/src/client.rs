    ...
    pub fn new(
        ...
        Self {
            state: Arc::new(ModelClientState {
                ...
                privacy_filter: PrivacyFilter::enabled_from_env()
                    .then(|| Arc::new(StdMutex::new(PrivacyFilter::default()))),
            }),
        }
    }
...
mod tests {
    ...
    fn privacy_filter_anonymizes_responses_request_input_before_send() {
        ...
        let privacy_filter = Arc::new(Mutex::new(PrivacyFilter::default()));
        ...
        let prompt = Prompt {
            input: vec![user_message(
                "Ask Alice Smith at alice.smith@example.com about 742 Evergreen Street.",
            )],
            ..Prompt::default()
        };
    ...
    async fn privacy_filter_restores_response_stream_items_before_downstream_display() {
        let privacy_filter = Arc::new(Mutex::new(PrivacyFilter::default()));
        let mut outbound = vec![user_message(
            "Contact Alice Smith at alice.smith@example.com.",
        )];
        privacy_filter
            .lock()
