...
impl ThreadRequestProcessor {
    ...
    async fn paginated_thread_turns_list_response(
        ...
        let loaded_thread = self.thread_manager.get_thread(thread_id).await.ok();
        let (has_live_running_thread, in_progress_turn_id) = match loaded_thread.as_ref() {
            Some(thread) => (
    ...
    async fn thread_fork_inner(
        ...
        // Paginated sources fork their reference-backed model context at the requested
        // boundary; rollout-backed sources follow the request's own history-mode contract.
