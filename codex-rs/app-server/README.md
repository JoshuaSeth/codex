...
- Begin a turn: To send user input, call `turn/start` with the target `threadId` and the user's input. Optional fields let you override model, cwd, sandbox policy or experimental `permissions` profile selection, approval policy, approvals reviewer, etc. This immediately returns the new turn object plus `submissionStatus` (`started` or `queued`) and, when known, the active turn id it queued behind. The app-server emits `turn/started` when that turn actually begins running.
...
- `turn/interrupt` — request cancellation of an in-flight turn by `(thread_id, turn_id)`; by default the response arrives after the turn reaches terminal state and includes `status: "interrupted"` or `status: "finished"` if it completed before the interrupt resolved. Set `respondImmediately: true` to receive `status: "requested"` as soon as cancellation is accepted; clients should still wait for `turn/completed` to observe final cleanup.
