...
async fn thread_settings_update_multi_agent_mode_applies_to_future_turns() -> Result<()> {
    ...
    assert_eq!(
        second_developer_texts
            .iter()
            .filter(|text| text.contains("Do not spawn sub-agents unless the user or applicable AGENTS.md/skill instructions explicitly ask for sub-agents, delegation, or parallel agent work."))
            .count(),
        1
    );
    Ok(())
