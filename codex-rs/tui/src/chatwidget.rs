...
impl ChatWidget {
    ...
    fn on_error_event(&mut self, error: ErrorEvent) {
    ...
    fn on_error(&mut self, message: String) {
        self.on_error_event(ErrorEvent {
            message,
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::Error(ev) => {
                let kind = ev.codex_error_info.as_ref().and_then(rate_limit_error_kind);
                if let Some(RateLimitErrorKind::ModelCap {
                    model,
                    reset_after_seconds,
                }) = kind
                {
