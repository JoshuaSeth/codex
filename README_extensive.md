...
- The production `send_email` tool (see `tools/custom_tools/send_graph_email.py`) now imports the real Graph client instead of shelling out. It calls `GraphEmailClient.send_email_with_metadata`, captures the Graph `message_id`/`internetMessageId`, and appends an audit line to `/Users/sethvanderbijl/PitchAI Code/mail_mcp/logs/elise_outbox_log.jsonl` while also maintaining an index file keyed by the `internet_message_id`. Each entry records `conversation_id`, `tool_call_id`, turn cwd, config file, and a pending/completed status so webhook automations know which rollout to resume. tools/custom_tools/send_graph_email.py:1-151standalone_mail_sender/graph_email.py:1-220
...
- `resume_elise_reply.py` looks up `{in_reply_to}` in the outbox index, patches the last pending tool result with the full reply body, and invokes `codex-dev exec resume <session> --replace-last-toolresult "..." "<auto prompt>"` from the captured working directory. Successful runs flip the entry to `status="completed"`; failures log stderr and keep the entry in `pending_reply` so a future webhook attempt (or a manual run of the script) can retry. The helper records JSONL traces under `logs/elise_resume_actions.jsonl` for auditing. infrastructure/outlook_webhook/hooks/resume_elise_reply.py:1-170
- Because Codex now injects `CODEX_CONVERSATION_ID`, `CODEX_TURN_ID`, and `CODEX_TURN_CWD` into every config-defined tool, any long-running automation can adopt the same pattern: persist metadata about the outgoing call, wire a webhook, and use `--replace-last-toolresult` to splice the final answer before resuming.
...
- **Pending custom tools** – Set `shutdown_after_call = true` inside any `[custom_tools.<name>]` stanza. When that tool runs, Codex captures its stdout/stderr, records the placeholder output in the rollout, emits a background note, then calls the standard shutdown path so the CLI exits cleanly. This is ideal for fire-and-forget workflows (send email, create Jira ticket, kick off pipelines) that only produce a final answer after some external system responds. See `codex-rs/core/src/tools/handlers/custom.rs` and `codex-rs/core/src/tools/registry.rs` for the plumbing.
...
- **Pinning the working directory** – Add `default_cwd = "/abs/path/to/project"` (or the profile-specific `default_cwd` key) to force Codex to operate from a known repository whenever the CLI launches without `--cd/-C`. Elise’s config now sets `default_cwd = "/Users/sethvanderbijl/PitchAI Code/mail_mcp"`, guaranteeing all of her agents run inside the mail automation repo, even when `codex-dev` is invoked from elsewhere.
- **Putting it together** – Our Elise automation is one concrete example, but the pattern is generic: (1) use `default_cwd` (global or per-profile) so the session always starts in the right repo; (2) enable `shutdown_after_call` on the tool that launches the external workflow; (3) have that tool log `{conversation_id, tool_call_id, metadata}` to disk; (4) wire a stop hook or webhook listener that, upon receiving the eventual answer, calls `codex exec resume <id> --replace-last-toolresult "…" "<follow-up prompt>"`. No manual steps are required once the notifications start flowing.
