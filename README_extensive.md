...
- Custom tools run inline: the tool call must complete before the turn continues. If you need async workflows, queue work externally and resume later with `codex exec resume` (optionally using `--replace-last-toolresult` to splice results into the rollout).
...
- `resume_elise_reply.py` resumes threads by invoking `codex-dev exec resume … --replace-last-toolresult "..." --no-prompt` with the reply payload captured by the webhook. infrastructure/outlook_webhook/hooks/resume_elise_reply.py:1-220
- For long-running automations, persist metadata (conversation id, call id, cwd) so an external runner can resume the thread later.
- Elise now exposes a `web_form_colleague` helper (`tools/custom_tools/web_form_colleague.py`) for browser automations. The tool queues a background run; the runner can resume the thread via `codex-dev exec resume …` once it has a summary.

### 5.7 Stop-hooks and resume helpers
...
- **Promptless resume (`--no-prompt`)** – `codex exec resume … --no-prompt` skips sending any new user input when restarting a rollout. Pair it with `--replace-last-toolresult` when you want Codex to continue from a patched tool output without adding a follow-up prompt. The flag is mutually exclusive with stdin/argument prompts; if you omit it, the CLI behaves exactly as before.
