...
- Once that external system finishes, run `codex-dev --replace-last-toolresult "<final text>" resume <SESSION_ID>` (or `codex exec resume --replace-last-toolresult "<text>" ...`) to splice the real tool output into the JSONL before Codex restarts the session. The flag refuses to run unless you target a specific rollout (explicit id or `--last`), which keeps “start fresh” sessions from being mutated accidentally.
