...
- `stop_hook_command` mirrors that idea but fires once per turn, right after the final assistant reply. The hook receives the conversation id, turn id, working directory, optional final message, raw `response_items`, and the latest token usage snapshot—handy for JSONL audit logs or webhooks. Configure it with the same syntax (for example `stop_hook_command = ["python3", "./tool_hook_logger.py", "~/.codex/aipc_turns.jsonl"]`). docs/config.md:375-405codex-rs/core/src/tools/hooks.rs:189-264
...
- **Transport:** Hooks are invoked via `tool_hook_command` (array of argv tokens). Codex writes the JSON payload to stdin and inherits stdout/stderr; non-zero exits are logged but never halt the agent. This makes it safe to point the hook at scripts that append to JSONL, forward to sockets, etc. codex-rs/core/src/tools/hooks.rs:1-80  To log straight from this repo, copy `tool_hook_logger.py` into your config and set `tool_hook_command = ["python3", "/path/to/tool_hook_logger.py", "/tmp/aipc_tool_calls.jsonl"]`. The logger now also respects the `CODEX_TOOL_HOOK_LOG` environment variable, so you can point multiple configs at different JSONL files without editing the script.
...
- Long-running automations now stay inside the live turn. Setting `hibernate_after_call = true` still runs the command and records its stdout/stderr, but instead of exiting the CLI we mark the tool call as *pending*, emit a spinner line (“pending: send_email waiting”), and park the turn until the final payload arrives. While Codex waits it drops a metadata file under `~/.codex/live/<conversation_id>.json` containing `{"host":"127.0.0.1","port":54321}` so webhook helpers know where to deliver the answer. This flow is covered by `cargo test -p codex-core custom_tool_hibernate_after_call_triggers_pending_flow`.
- When the external system finishes, call `codex-dev exec deliver-pending <SESSION_ID> --call-id <CALL_ID> --output "Final text" --success/--no-success`. The helper reads the `~/.codex/live/<id>.json` descriptor, connects to the waiting CLI, and sends a `DeliverPendingToolResult` op so the model receives the real tool output immediately—no fake follow-up prompt and no “resume” loop. If the CLI has already exited, fall back to `codex-dev exec resume <SESSION_ID> --replace-last-toolresult "<final text>" --no-prompt` to edit the rollout manually.
...
### 5.6 Elise auto-resume pipeline
- The production `send_email` tool (see `tools/custom_tools/send_graph_email.py`) now returns immediately after logging the outbound metadata. It still calls `GraphEmailClient.send_email_with_metadata`, but instead of hibernating it echoes the Graph and internet ids so the agent can decide whether to wait. The shared outbox index (under `/Users/sethvanderbijl/PitchAI Code/Elise/core/logs/elise_outbox_index.json`) records `conversation_id`, `tool_call_id`, cwd, config file, and URL metadata for every internet id. tools/custom_tools/send_graph_email.py:1-170standalone_mail_sender/graph_email.py:1-220
- **Checking the mailbox inside Codex** – `mail_search` (`tools/custom_tools/mail_search.py`) queries Outlook directly so Elise can list unread messages (subject, sender, received time, previews, message ids) without leaving the CLI. `mail_read` (`tools/custom_tools/mail_read.py`) consumes either the Graph `message_id` or `internet_message_id`, returns the full body (HTML or normalized text), and can optionally mark the message as read once she’s reviewed it. tools/custom_tools/mail_search.py:1-150tools/custom_tools/mail_read.py:1-140
- The Outlook webhook server fetches richer message data (`body`, `internetMessageHeaders`, `conversationId`) and the dispatcher emits context fields such as `in_reply_to`, `body_html`, and `body_content_type`. That data feeds the new hook declared in `infrastructure/outlook_webhook/actions_config.json`, so every incoming mail to `elise@pitchai.net` runs `hooks/resume_elise_reply.py`. infrastructure/outlook_webhook/outlook_webhook_server.py:1-86infrastructure/outlook_webhook/action_dispatcher.py:1-210infrastructure/outlook_webhook/actions_config.json:1-60
- `resume_elise_reply.py` still prefers `codex-dev exec deliver-pending …` when a waiting call id is recorded (useful for any profile that continues to hibernate pending tools). If no live session is waiting, it falls back to `codex-dev exec resume … --replace-last-toolresult "..." --no-prompt`. JSONL traces still land in `logs/elise_resume_actions.jsonl`. infrastructure/outlook_webhook/hooks/resume_elise_reply.py:1-220
- Because Codex injects `CODEX_CONVERSATION_ID`, `CODEX_TURN_ID`, and `CODEX_TURN_CWD` into every config-defined tool, any long-running automation can adopt this pattern: persist metadata about the outgoing call, use a `wait_*` helper with `hibernate_after_call = true`, and have the webhook invoke `deliver-pending` with the stored conversation/call id.
- Elise now exposes a `web_form_colleague` helper (`tools/custom_tools/web_form_colleague.py`) for browser automations. The tool logs tasks to `PitchAI Code/Elise/core/logs/elise_web_tasks.json`, launches `infrastructure/web_tasks/run_web_agent_task.py`, and returns immediately with `hibernate_after_call = true`. The runner executes `codex-dev --config-file ~/.codex/web-agent.toml exec --skip-git-repo-check "<prompt>"`, captures the Chrome DevTools output, and finally calls `codex-dev exec deliver-pending <conversation_id> --call-id <tool_call_id> --output "<web summary>"` using the metadata published to `~/.codex/live/<conversation_id>.json`. Elise’s turn resumes in-place with the returned summary—no extra resume CLI, no synthetic prompt.

### 5.7 Stop-hooks, pending tools, and resume helpers
- **Stop hooks** – Configure `stop_hook_command = ["python3", "./tools/custom_tools/telegram_bot.py", "--stop-hook"]` to run arbitrary scripts whenever Codex finishes a turn. The hook receives `{conversation_id, turn_id, cwd, final_message, response_items, token_usage}` so you can push summaries to chat, update dashboards, or trigger pager alerts. Implementation lives in `codex-rs/core/src/tools/hooks.rs:189-264` and is fully documented in `docs/config.md#tool_hook_command`.
- **Pending custom tools** – Set `hibernate_after_call = true` inside any `[custom_tools.<name>]` stanza. Codex runs the command, streams its stdout/stderr like any other tool, and then pauses the turn with a “pending” spinner while the external workflow finishes. The exec binary writes connection info to `~/.codex/live/<conversation_id>.json`, so your webhook (or a teammate) can call `codex-dev exec deliver-pending …` to provide the final payload. This remains useful for long-running helpers such as the `web_form_colleague` automation; if the CLI isn’t running anymore, fall back to `--replace-last-toolresult` plus `--no-prompt`.
- **Replacing the last tool result** – Both the interactive CLI and `codex exec` accept `--replace-last-toolresult "TEXT"` when resuming. The flag edits the most recent `FunctionCallOutput`/`CustomToolCallOutput` entry in the rollout before the session restarts, so Codex sees the “real” result the next time it prompts the model. The helper lives in `codex-rs/core/src/rollout/edit.rs` and is surfaced via `codex resume --id … --replace-last-toolresult` (TUI) and `codex exec resume --last --replace-last-toolresult` (exec mode).
- **Promptless resume (`--no-prompt`)** – `codex exec resume … --no-prompt` now skips sending any new user input when restarting a rollout. Pair it with `--replace-last-toolresult` for pending automations so Codex simply consumes the patched tool output and keeps reasoning. This is how both the Graph webhook (`resume_elise_reply.py`) and the web-form colleague runner resume Elise: they splice the real tool output, invoke `codex-dev exec … --replace-last-toolresult "…" --no-prompt`, and the agent immediately continues with no additional instructions. The flag is mutually exclusive with stdin/argument prompts; if you omit it, the CLI behaves exactly as before.
- **Prompt sequences** – Pass `--prompt-sequence /path/to/sequence.toml` to `codex exec` and the CLI will pull prompts (and optional attachments) from the `[[steps]]` array, submit them one after another, and only exit once the final step completes. This is perfect for “collect context → draft → summarize” style workflows without manually copy/pasting intermediate instructions. The example at `docs/prompt_sequences/two_step_demo.toml` demonstrates two tightly scoped prompts; add as many steps as needed. The flag can’t be combined with ad-hoc PROMPT arguments, `--image`, or exec subcommands—the sequence file is the source of truth.
- **Pinning the working directory** – Add `default_cwd = "/abs/path/to/project"` (or the profile-specific `default_cwd` key) to force Codex to operate from a known repository whenever the CLI launches without `--cd/-C`. Elise’s config now sets `default_cwd = "/Users/sethvanderbijl/PitchAI Code/Elise/core"`, guaranteeing all of her agents run inside the consolidated Elise automation repo, even when `codex-dev` is invoked from elsewhere.
- **Putting it together** – Our Elise automation is one concrete example, but the pattern is generic: (1) use `default_cwd` (global or per-profile) so the session always starts in the right repo; (2) run the actual work (send email, launch a web agent, kick off CI) in ordinary tools that return immediately; (3) call a tiny `wait_*` helper that sets `hibernate_after_call = true` and records the pending metadata; (4) wire a stop hook or webhook listener that, upon receiving the eventual answer, calls `codex exec deliver-pending …` (or falls back to `resume --replace-last-toolresult`). No manual steps or placeholder user prompts are required once the notifications start flowing.
